...
#
# Gera:
#   dist/galois-library-<ver>.tar.gz   (raíz = app: .lua, sources/, koplugin/, install.sh)
...
cp -r sources settings.lua catalog.lua health.lua json.lua net_curl.lua \
      update.lua cli.lua install.sh koplugin README.md "$STAGE/" 2>/dev/null
printf '%s\n' "$VER" > "$STAGE/version"
...
printf '{"tag_name":"v%s","tarball_url":"dist/%s"}\n' "$VER" "$OUT" > dist/release.json
